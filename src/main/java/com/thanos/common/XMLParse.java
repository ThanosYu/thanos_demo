...
public class XMLParse {
	...
	public static void main(String[] args) {
		...
		xp.parseXML("d://sample", "UTF-8");
		xp.search("//message");
		if (xp.next()) {
			String username = xp.getNodeValue("username");
			String password = xp.getNodeValue("password");
			String content = xp.getNodeValue("content");
			List list = xp.getSubElementList("mobiles");
			Iterator iter = list.iterator();
			if (iter.hasNext()) {
				Element elem = (Element) iter.next();
				list = elem.elements("mobile");
			}
			// List list = xp.getEl().elements("mobile");
